    ...
    def evaluate_celebrities(self, celebrity_dir='.'):
        ...
        for img_name in os.listdir(celebrity_dir):                              # go through all the celebrity images in the folder
            ...
            for top_id in top5_ids:
                ...
                subface_id = self.training_ids[int(face_id-1)][top_id % self.train_faces_count]           # getting the exact subimage from the face
                
                path_to_img = os.path.join(self.faces_dir,
                        's' + str(int(face_id)), str(subface_id) + '.pgm')           # relative path to the top5 face
...
if __name__ == "__main__":
    ...
    efaces = Eigenfaces("att_faces")                                       # create the Eigenfaces object with the data dir
    efaces.evaluate()                                                           # evaluate our model

    #if len(sys.argv) == 3:                                                      # if we have third argument (celebrity folder)
    efaces.evaluate_celebrities("celebrity_faces")                           # find best matches for the celebrities
